...
static int16_t get_y(int16_t y) {
  ...
  int16_t offset = (unobstructed_bounds.size.h - bounds.size.h) / 4;

  if (y >= 0) {
    return y + offset;
  } else {
    return unobstructed_bounds.size.h + y + offset;
  }
}
...
static void display_layer_update_callback(Layer *layer, GContext *ctx) {
  ...
  graphics_draw_rect(ctx, GRect(0, get_y(S_Y_LINE), bounds.size.w, 2));
}

static void window_load(Window *window) {
  s_window_layer = window_get_root_layer(window);
  bounds = layer_get_unobstructed_bounds(s_window_layer);

  s_timestamp_layer = text_layer_create(GRect(3, get_y(S_Y_TIMESTAMP), bounds.size.w - 6, 16));
  ...
  layer_add_child(s_window_layer, text_layer_get_layer(s_timestamp_layer));

  s_date_layer = text_layer_create(GRect(3, get_y(S_Y_DATE), bounds.size.w - 6, 20));
  ...
  s_day_layer = text_layer_create(GRect(bounds.size.w - 53, get_y(S_Y_DAY), 50, 16));
  ...
  s_time_layer = text_layer_create(GRect(0, get_y(S_Y_TIME), bounds.size.w, 50));
